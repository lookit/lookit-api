...
class OrganizationTestCase(APITestCase):
    def setUp(self):
        ...
        self.url = reverse('organization-list',  kwargs={'version':'v1'}) + str(self.organization.uuid) + '/'
    ...
    def testPostOrganization(self):
        self.client.force_authenticate(user=self.researcher)
        api_response = self.client.post(reverse('organization-list',  kwargs={'version':'v1'}), content_type="application/vnd.api+json")
        self.assertEqual(api_response.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)

    def testUpdateOrganization(self):
        self.client.force_authenticate(user=self.researcher)
        api_response = self.client.patch(self.url, content_type="application/vnd.api+json")
        self.assertEqual(api_response.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)

    def testDeleteOrganization(self):
        self.client.force_authenticate(user=self.researcher)
        api_response = self.client.delete(self.url, content_type="application/vnd.api+json")
        self.assertEqual(api_response.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)
