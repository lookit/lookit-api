...
class StudyResponsesList(StudyResponsesMixin, generic.DetailView, PaginatorMixin):
	...
	def build_individual_csv(self, responses):
		...
		"""
		csv_responses = []
		standard_headers = self.get_ordered_csv_headers_for_row_data()
		for resp in responses:
			row_data = self.get_csv_headers_and_row_data(resp)
			headerList = standard_headers + sorted(list(set(row_data.keys()) - set(standard_headers)))
