...
class RenameVideoView(View):
	...
	def post(self, request):
		...
		postbody = urllib.parse.parse_qs(bytes(rawbody).decode('utf-8')) # NOW parse the request string
		...
		d = ast.literal_eval(postbody['payload'][0]) # convert from string representation of dict
			
		if authenticated and (d['data']['s3UploadStatus'] == "upload success"): # Go ahead and move the file
			...
			if success:
				return HttpResponse(d['data']['videoName'] + ' --> ' + d['data']['payload'])
			else:
