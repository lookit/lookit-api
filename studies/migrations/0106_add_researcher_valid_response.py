...
def compute_is_tallied(apps, schema_editor):
    ...
    """
    response_model = apps.get_model("studies", "Response")
    consent_ruling_model = apps.get_model("studies", "ConsentRuling")
    ...
    newest_ruling_subquery = models.Subquery(
        consent_ruling_model.objects.filter(response=models.OuterRef("pk"))
        .order_by("-created_at")
        .values("action")[:1]
    )
    tallied_internal_ids = list(
        response_model.objects.exclude(study__study_type_id=EXTERNAL_STUDY_TYPE_ID)
    ...
    if tallied_internal_ids:
        response_model.objects.filter(id__in=tallied_internal_ids).update(
            is_tallied=True
        )
